...
got a few, I'd then CTRL+F in Notepad++, navigate to the "Replace" tab & input the following into the "Find What" box: `.*(nameoftopdomainhere.com)` & then making sure to put "$1" in the "Replace With:" box. This allowed me to quickly, albeit kind of dirtily, remove hundreds of subdomains at once, leaving behind nothing but multiple lines of the top-level domain. 
